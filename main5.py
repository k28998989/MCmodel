...
for epoch in range(EPOCH):
    cnn.train()
    for step, (x, y, w, h) in enumerate(data_loader):
        w = w.to(device)
        h = h.to(device)
        tem_label=[]
        for i in y:
    ...
    test_x, test_y, w, h = next(iter(test_loader))
    tem_label = []
    for i in test_y:
        if i == 'down':
            tem_label.append(0)
    ...
    cnn.eval()
    test0 = cnn(test_x.to(device), w.to(device), h.to(device))
    print(nn.Softmax()(test0))
...
for i in test_y:
    if i == 'down':
        ...
        tem_label.append(7)
test_y = torch.tensor(tem_label).to(device)
text_x = test_x.to(device, w.to(device), h.to(device))
cnn.eval()
